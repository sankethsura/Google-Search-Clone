...
const SearchPage2 = (props) => {
  ...
  //   const arr =[]
  console.log(props.data.items[0].snippet);
    ...
    return (
      <div key={i}>
        <div  className="flex items-start flex-col w-[70vw] m-auto pt-4">
          ...
          <div onClick={() => window.open(item.link)} className="text-start text-[#2155CD] font-semibold text-xl py-1 hover:underline cursor-pointer">
            {item.title}
          </div>
    ...
    <div>
      <nav className="flex justify-between items-center">
        <div className="flex items-center ">
            <div onClick={()=>{nav("/")}} className="pb-[10px] px-[45px] scale-150 cursor-pointer"><IoIosArrowBack /></div>
          <img
          onClick={()=>{nav("/")}}
